...
asa_acl = namedtuple('asa_acl', 'srcintf dstintf srcip srcport dstip dstport action comments status')
    ...
    for line in acl:
        ...
        for key in ['srcip', 'dstip']:
            ...
            if lines[index] == 'any':
                ...
                index += 1
                src_dst[key] = check_addr(ip.format(lines[index],'255.255.255.255'))
        ...
        if proto in ['udp','tcp']  and index<len(lines):
            ...
            if dport_type == 'eq':
                ...
                if dstport in obj_ser_grp:
                    dstport_details  = [ser for ser in obj_ser_grp[dstport].split() if ser.startswith('tcp')]
                else:
                ...
                if proto == 'tcp':
                    ...
                    dstport_details = ['tcp-'+str(x) for x in range(int(s_port),int(e_port))]
        ...
        if not src_dst['dstip'].startswith('n-') and src_dst['dstip'] != 'any':
            if src_dst['dstip'].startswith('h-'):
                ...
                dips = [ net for net in obj_net_grp[src_dst['dstip']].split() if net.startswith('h-') ]
                no_change = 0
            ...
            for dip in dips:
                ...
                if NAT_target in bi_vip_list:
                    ...
                    for nat_result in bi_vip_list[NAT_target]:
                        ...
                        nat_dst_ip += ' ' + check_addr(ip.format(nat_result.split('#')[1],'255.255.255.255'))
                    result.append(asa_acl(srcintf,nat_dst_intf,src_dst['srcip'],srcport,nat_dst_ip,dstport,action,comments+nat_commnent.format(NAT_target,nat_to),nat_status))
                    ...
                    for dport in dstport_details:
                        ...
                        if PAT_target in bi_vip_list:
                            ...
                            for nat_result in bi_vip_list[PAT_target]:
                                nat_dst_intf += ' ' +nat_result.split('#')[0]
                                nat_dst_ip += ' ' + check_addr(ip.format(nat_result.split('#')[1],'255.255.255.255'))
                                nat_dst_port += ' ' + check_service('tcp',nat_result.split('#')[2])
                            result.append(asa_acl(srcintf,nat_dst_intf,src_dst['srcip'],srcport,nat_dst_ip,nat_dst_port,action,comments+nat_commnent.format(PAT_target,pat_to),nat_status))
                            orig_status = 'disable'

        result.append(asa_acl(srcintf,'any',src_dst['srcip'],srcport,src_dst['dstip'],dstport,action,comments,orig_status))
    return result
...
for line in asa_config:
                        ...
                        for l4 in l4_type_line.split('-'):
                            if ' eq ' in line:
                                ...
                                if obj_name not in service_list:
                                    service_list[obj_name] = l4 + '-portrange ' + port_no

                            elif ' range ' in line:
                                port_no_start = check_port(lines[2+offset])
                                port_no_end = check_port(lines[3+offset])
                                obj_name = '{}-{}_{}'.format(l4,port_no_start,port_no_end)
                                if obj_name not in service_list:
                                    service_list[obj_name] = '{}-portrange {}-{}'.format(l4,port_no_start,port_no_end)
    ...
    if re_static_nat.match(line):
        ...
        if re_ip.match(lines[2]):
            ...
            if re_ip.match(lines[5]):
                ...
                in_port = '404'
            nat = 'PAT_{}_{}_{}_{}'.format(in_ip,in_port,out_ip,out_port)
            portforward = True
            bi_vip_list[out_intf+'#'+out_ip+'#'+out_port] = in_intf+'#'+ in_ip+'#'+ in_port
    ...
    if re_route.match(line):            
        pass
#access-group PEGASUS-CDHB_INBOUND in interface PEGASUS-CDHB
...
#for acl in acl_list:
#    analyze_acl(acl,acl_list[acl])

compents = [c_addr, c_service, c_addrgrp, c_sergrp, c_ippool, c_vip, c_snat, c_policy]
for compent in compents:
    file_name = 'forticonvert_{}.txt'.format(compent.name)
    with open(file_name,'w') as f:
        f.write(jenv.get_template(compent.template).render(data = compent.data, vdom = 'Pegasus'))
